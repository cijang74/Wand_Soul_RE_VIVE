...
public class ThunderLaser : MonoBehaviour
{
    ...
    private void OnTriggerEnter2D(Collider2D other) 
    {
        ...
        // 만약 닿은게 파괴불가능한 오브젝트이고 그것이 Trigger가 아니라면 (나무의 나뭇잎 때문에)
        {
            ...
            myAnimator.SetTrigger("IsTouchGround");
        }
    }

    private void SetMaxTrigger()
    {
        myAnimator.SetTrigger("IsThunderMax");
    }
    ...
    private IEnumerator IncreaseLaserLengthRoutine()
    {
        ...
        while(spriteRenderer.size.y < laserRange && isGrowing == true)
        // 스프라이트의 y축 크기가 레이저 범위까지 다다를때까지 반복
        {
            ...
            capsuleCollider2D.offset = new Vector2(capsuleCollider2D.offset.x, Mathf.Lerp(1f, laserRange, linearT) / 2);
            // offset은 한쪽방향으로만 이동하는거니까 사실상 /2해주어야 함.
            transform.position = new Vector2(transform.position.x, y - Mathf.Lerp(1f, laserRange, linearT) / 2);
